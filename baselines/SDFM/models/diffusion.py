...
class Diffusion:
    ...
    def denoise_motion(self, x_t, t, noise):
        sqrt_alpha_hat = torch.sqrt(self.alpha_hat[t])[:, None, None]
        sqrt_one_minus_alpha_hat = torch.sqrt(1 - self.alpha_hat[t])[:, None, None]
    ...
    def inpaint_complete(self, step, x, prev_t, traj_dct, mode_dict, mask=None):
        """
        ...
        if mode_dict['mode'] == 'control':
            ...
            x_prev_t_known, _ = self.noise_motion(traj_dct, prev_t)  # origin traj add noise in DCT domain

        if not self.use_dct:
            """
            不使用DCT 模型输出x和输入轨迹traj_dct都在时域 traj_dct加噪声后与x拼接
            """
            if mask is None:
                mask = mode_dict['mask']

            x = torch.mul(mask, x_prev_t_known) + torch.mul((1 - mask), x)  # mask
            return x

        # B, n_total, D
        x_prev_t_known = torch.matmul(self.idct[:, :self.n_pre], x_prev_t_known[:, :self.n_pre])  # return time domain for mask
        # B, n_total, D
        x_prev_t_unknown = torch.matmul(self.idct[:, :self.n_pre], x[:, :self.n_pre])  # 模型输出和原始输入变换回时域进行拼接
        ...
        x = torch.matmul(self.dct[:self.n_pre], x)  # DCT变换到n_pre帧

        return x

    def sample_ddim_progressive(self, model, traj_dct, traj_dct_mod, mode_dict, noise=None, mask=None,
                                global_feature=None, vel_acc_pad=None):
        """
        ...
        with torch.no_grad():
            for i in reversed(range(0, self.ddim_timesteps)):
                ...
                posterior_variance_t = self.beta[t][:, None, None] * (1. - alpha_hat_prev) / (1. - alpha_hat)
                beta = self.beta[t][:, None, None]

                if mode_dict['mode'] != 'autoregression':
                    if vel_acc_pad is not None:
                        predicted_noise = model(x, t, mod=traj_dct_mod, vel_acc=vel_acc_pad)  # 模型输入输出n_pre帧
                    else:
                        predicted_noise = model(x, t, mod=traj_dct_mod)
                else:
                    predicted_noise = model(x, global_feature, t)

                # x0_ = (x_t - sqrt(1 - a_t) * e_) / sqrt(a_t)
                ...
                # if use DCT, DCT domain else temporal domain
                x = x_prev
                """

                # mean + noise
                mean = (x - (beta / torch.sqrt(1. - alpha_hat)) * predicted_noise) / torch.sqrt(1. - beta)
                if i == 0:
                    z = 0.
                else:
    ...
    def sample_ddim_autoregression(self,
                                   model,
                                   cond_traj,
                                   global_feature,
                                   mode_dict,
                                   mask=None):
        # cond_traj (B, clip_his, 3V)
        final = None
        for sample in self.sample_ddim_progressive(model,
                                                   cond_traj,
                                                   None,
