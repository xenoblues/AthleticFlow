...
class TrainerAutoregression:
    def __init__(self,
        ...
        if self.cfg.ema is True:
            ...
            self.ema_encoder = copy.deepcopy(encoder).eval().requires_grad_(False).cuda()
            self.ema_setup = (self.cfg.ema, self.ema, self.ema_model, self.encoder)
        else:
            self.ema_model = None
    ...
    def before_train(self):
        ...
        self.optimizer = optim.Adam([{"params": self.model.parameters(), "lr": self.cfg.lr},
    ...
    def run_train_step(self):
        ...
        for traj_np, mask in self.generator_train:
            start_pos = torch.randint(0, self.cfg.t_total - 2 * self.cfg.clip_total, (1,))
            with torch.no_grad():
            ...
            for s in range(self.stages):
                with torch.no_grad():
                    start_index = s * self.cfg.clip_total - s * self.cfg.clip_his + start_pos
                    end_index = (s + 1) * self.cfg.clip_total - s * self.cfg.clip_his + start_pos
                    # 取当前clip轨迹
                    s_traj = traj[:, start_index:end_index, :].clone()

                    if s == 0:  # stage1 历史帧为真实轨迹的前clip_his帧
                        # 预测结果轨迹，stage1时为0
                        pred_traj = torch.zeros((traj.shape[0], self.cfg.clip_total, traj.shape[-1]),
                                                device=traj.device, dtype=self.cfg.dtype)
                        traj_his = s_traj[:, :self.cfg.clip_his, :].clone()
                    else:  # stage2 历史帧为stage1预测结果轨迹的后clip_his帧
                        traj_his = pred_traj[:, -self.cfg.clip_his:, :].clone()

                    # 拼接历史帧和预测帧作为输入
                    input_traj = torch.cat((traj_his, s_traj[:, self.cfg.clip_his:, :]), dim=1)


                    if self.cfg.residual_data:  # 使用速度作为输入
                        res_traj = torch.zeros(s_traj.shape, device=self.cfg.device, dtype=self.cfg.dtype)
                        res_traj[:, 1:, :] = s_traj[:, 1:, :] - s_traj[:, :-1, :]
                        s_traj = res_traj
                    ...
                    if self.cfg.random_sample:
                        input_traj = torch.zeros((s_traj.shape[0], self.cfg.n_pre, s_traj.shape[-1]))
                        for i in range(s_traj.shape[0]):
                            a = np.arange(s_traj.shape[1])
                            np.random.shuffle(a)
                            a = np.sort(a[:self.cfg.n_pre])
                            input_traj[i, :, :] = s_traj[i, a, :]

                    if self.cfg.use_dct:
                        traj_dct = torch.matmul(self.cfg.dct_m_all[:self.cfg.n_pre], s_traj)
                ...
                if self.cfg.frame_mask:
                    # 随机掩膜，选择被加噪声的帧
                    frame_mask = torch.ones((input_traj.shape[0], self.cfg.n_pre, 1)).cuda().detach()
                    frame_mask = torch.dropout(frame_mask, 0.8, True)
                    frame_mask[frame_mask > 0] = 1.0
                if self.cfg.joint_mask:
                    joint_mask = torch.ones(
                        (input_traj.shape[0], self.cfg.n_pre, input_traj.shape[-1] // 3)).cuda().detach()
                ...
                # train
                t = self.diffusion.sample_timesteps(input_traj.shape[0]).to(self.cfg.device)
                x_t, noise = self.diffusion.noise_motion(input_traj, t)  # 加噪声
                x_t = input_traj.mul(mask) + x_t.mul(1.0 - mask)  # 混合历史帧和噪声


                end_time1 = time.time()
                # print("生成Mask耗时:{:.5f}秒".format(end_time1 - start_time))
                global_feature = self.encoder(pred_traj)
                end_time2 = time.time()
                # print("编码计算耗时:{:.5f}秒".format(end_time2 - end_time1))
                predicted_noise = self.model(x_t, global_feature, t)
                pred_traj = self.diffusion.denoise_motion(x_t, t, predicted_noise).detach()

                end_time3 = time.time()
                # print("模型计算耗时:{:.5f}秒".format(end_time3 - end_time2))
                ...
                end_time4 = time.time()
                # print("loss计算耗时:{:.5f}秒".format(end_time4 - end_time3))
                ...
                self.optimizer.step()
                end_time5 = time.time()
                # print("反向传播耗时:{:.5f}秒".format(end_time5 - end_time4))
                ...
                args_ema, ema, ema_model, ema_encoder = self.ema_setup
                if args_ema is True:
                    ema.step_ema(ema_model, self.model)
                    ema.step_ema(ema_encoder, self.encoder)

                end_time6 = time.time()
                # print("stage {:d} 耗时:{:.5f}秒".format(s, end_time6 - start_time))
                # del
            del pred_traj, traj, traj_np, loss, traj_dct, traj_dct_mod, mask
    ...
    def after_train_step(self):
        ...
        if self.iter % self.cfg.save_gif_interval == 0:
            pose_gen = pose_generator(self.dataset['train'], self.model, self.diffusion, self.cfg,
                                      mode='gif_regression', encoder_select=self.encoder)
        ...
        for traj_np, mask in self.generator_val:
            ...
            for s in range(self.stages):
                with torch.no_grad():
                    start_index = s * self.cfg.clip_total - s * self.cfg.clip_his + start_pos
                    end_index = (s + 1) * self.cfg.clip_total - s * self.cfg.clip_his + start_pos
                    s_traj = traj[:, start_index:end_index, :].clone()  # 取当前clip轨迹
                    ...
                    # traj_pad = padding_traj(s_traj, self.cfg.padding, self.cfg.idx_pad, self.cfg.zero_index)
                    ...
                    t = self.diffusion.sample_timesteps(input_traj.shape[0]).cuda()
                    x_t, noise = self.diffusion.noise_motion(input_traj, t)  # 加噪声
            ...
            del pred_traj, traj, traj_np, loss, traj_dct, traj_dct_mod, mask, input_traj
    ...
    def after_val_step(self):
        ...
        if self.iter % self.cfg.save_gif_interval == 0:
            if self.cfg.ema is True:
                pose_gen = pose_generator(self.dataset['test'], self.ema_model, self.diffusion, self.cfg,
                                          mode='gif_regression', encoder_select=self.ema_encoder)
            else:
        ...
        if self.iter % self.cfg.save_metrics_interval == 0 and self.iter != 0:
            if self.cfg.ema is True:
                compute_stats(self.diffusion, self.multimodal_dict, self.ema_model, self.logger, self.cfg,
                              encoder_model=self.ema_encoder)
            else:
        ...
        if self.cfg.save_model_interval > 0 and (self.iter + 1) % self.cfg.save_model_interval == 0:
            if self.cfg.ema is True:
                ...
                torch.save(self.ema_encoder.state_dict(),
                           os.path.join(self.cfg.model_path, f"ckpt_emaec_{self.iter + 1}.pt"))
            else:
