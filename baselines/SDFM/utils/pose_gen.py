        ...
        for k in range(0, nrow):
            if mode == 'switch':
                ...
                if cfg.dataset == 'h36m' or cfg.dataset == 'humaneva' or cfg.dataset == 'assemble':
                    ...
                    sampling_generator = data_set.sampling_generator(num_samples=256, batch_size=256, aug=False)
                    data, _ = next(sampling_generator)
            ...
            if not cfg.autoregression:
                ...
                his_range = [i for i in range(cfg.clip_his)]
                mask = torch.zeros_like(traj_dct).to(cfg.device)
                mask[:, his_range, :] = 1
                # last_feature = torch.zeros((traj_dct.shape[0], cfg.clip_total, cfg.latent_dims)).to(cfg.device)
                sampled_motion = clips_prediction(traj_dct, cfg, generator_model, encoder_select, model_select, mode_dict)
