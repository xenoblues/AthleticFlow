...
from models import KAFormer, SGTransformer, athletic_flow
...
class Trainer_fm:
    ...
    def before_train(self):
        ...
        # self.dct_derivative_mat = precompute_dct_derivative_matrix(self.cfg.t_total, self.cfg.n_pre).to(self.cfg.device)
        # self.freq_weights = torch.linspace(1.0, 3.0, self.cfg.n_pre, device=self.cfg.device)
        # self.dyn_loss_fn = SpectralDynamicsLoss().to(self.cfg.device)

        # self.dct_m_vel, self.idct_m_vel = util.get_dct_matrix(self.cfg.t_pred + self.cfg.t_his - 1)
        # self.dct_m_vel = self.dct_m_vel.float().to(self.cfg.device)
        # self.idct_m_vel = self.idct_m_vel.float().to(self.cfg.device)
        #
        # self.dct_m_acc, self.idct_m_acc = util.get_dct_matrix(self.cfg.t_pred + self.cfg.t_his - 2)
        # self.dct_m_acc = self.dct_m_acc.float().to(self.cfg.device)
        # self.idct_m_acc = self.idct_m_acc.float().to(self.cfg.device)

        # self.criterion_fm = JointWeightedFlowMatchingLoss().to(self.cfg.device)
    ...
    def run_train_step(self):
        for traj_np, mask in self.generator_train:
            ...
            with (torch.amp.autocast(dtype=torch.bfloat16, device_type="cuda")):
                ...
                if isinstance(self.model, KAFormer.KASDFMTransformer):
                    limb_assignment = self.model.limb_fuser.limb_clustering()
                    reg_loss = limb_regularization_loss(limb_assignment)
                    loss = loss + reg_loss * 0.01
                elif isinstance(self.model, SGFormer):
                    pred_ut_cart = torch.matmul(self.cfg.idct_m_all[:, :self.cfg.n_pre], pred_ut[:, :self.cfg.n_pre])
                    x_t_cart = torch.matmul(self.cfg.idct_m_all[:, :self.cfg.n_pre], x_t[:, :self.cfg.n_pre])
                    loss_cons = velocity_physical_consistency_loss(pred_ut_cart, x_t_cart)
                    loss = loss + loss_cons
                elif isinstance(self.model, SGFormerLite):
                    # loss_bio = biomechanics_loss(pred_traj.float(), traj)
                    # self.logger.info('{:.8f} lr: {:.8f}'.format(loss.item(), loss_bio.item()))
                    # loss = loss + loss_bio
                    pass
                elif isinstance(self.model, VKDFFormer):
                    mu, sigma = self.model.get_mu_sigma()
                    loss = hard_example_weighted_loss(pred_ut.float(), u_t.float(), mu, sigma, x_t,
                                                      self.dct_derivative_mat, self.freq_weights)
                elif isinstance(self.model, AthleticFlow):
                    loss_traj = F.smooth_l1_loss(pred_traj, traj)
                    # loss_motion = compute_motion_losses(pred_traj, traj)
                    # loss = loss + loss_motion
                    # loss_traj = weighted_traj_loss(pred_traj,  traj)
                    # loss_vel = weighted_velocity_loss(pred_traj, traj)
                    # loss_ee =  end_effector_loss(pred_traj, traj)
                    # loss_bone_dir, loss_chain_dir = limb_chain_edge_fm_loss(pred_traj, traj)
                    # self.logger.info('l_fm:{:.8f} loss_bone_dir: {:.8f} loss_chain_dir: {:.8f}'
                    #                  .format(loss.item(), loss_bone_dir.item(), loss_chain_dir.item()))
                    # loss = loss + 0.01 * loss_bone_dir + 0.005 * loss_chain_dir


                if self.cfg.mode == 'fine_tune':
                    refined_traj, _ = self.generator.refiner(pred_traj.float().detach(), latent.detach())
                    loss, loss_dir, loss_pos = limb_chain_direction_distill_loss(pred_traj.float().detach(), refined_traj,
                                                                                 traj, t_pred=self.cfg.t_pred,
                                                                                 num_joints=self.cfg.joint_num)
        ...
        for traj_np, mask in self.generator_val:
            with torch.no_grad():
                ...
                if isinstance(self.model, KAFormer.KASDFMTransformer):
                    ...
                    # loss_traj = weighted_traj_loss(pred_traj,  traj)
                    # loss_vel = weighted_velocity_loss(pred_traj, traj)
                    # loss_bone_dir, loss_chain_dir = kinematic_direction_loss(pred_traj, traj)
                    # self.logger.info('l_fm:{:.8f} loss_bone_dir: {:.8f} loss_chain_dir: {:.8f}'
                    #                  .format(loss.item(), loss_bone_dir.item(), loss_chain_dir.item()))
                    # loss = loss + 0.01 * loss_bone_dir + 0.005 * loss_chain_dir
                    # err0 = u_t.float() - v0.detach()
                    # loss_r1 = nn.functional.mse_loss(r1.float(), err0.float())
                    # err1 = err0 - r1.detach()
                    # loss_r2 = nn.functional.mse_loss(r2.float(), err1.float())
                    # loss = loss + 0.5 * loss_r1 + 0.25 * loss_r2
