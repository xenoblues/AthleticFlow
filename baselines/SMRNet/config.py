...
def update_config(cfg, args_dict):
    ...
    # The previous `get_log_dir_index` counter (len(os.listdir(base_dir))) is not
    # collision-safe: two processes that list the directory before either has
    # created its own subdirectory compute the same index and silently share it
    # (os.makedirs(..., exist_ok=True) never errors). This has already happened
    # in practice (e.g. results/ap3d_6 and results/wp_5, each overwritten in
    # place by a later colliding run). Use a timestamp+seed+PID suffix instead,
    # which is unique per process regardless of scheduling races.
    import time
    formatted_time = time.strftime('%Y%m%d-%H%M%S', time.localtime())
    seed = args_dict.get('seed', 'NA')
    cfg.cfg_dir = '%s/%s_seed%s_%s_%d' % (cfg.base_dir, args_dict['cfg'], seed, formatted_time, os.getpid())
    ...
    cfg.t_total = cfg.t_his + cfg.t_pred
...
class Config:
    ...
    def __init__(self, cfg_id, test=False):
        ...
        self.remove_root = cfg.get('remove_root', False)

        # indirect variable
        if self.dataset == 'h36m' or self.dataset == 'ap3d' or self.dataset == 'ap':
            self.joint_num = 16
