...
def get_fps(dataset_name, cfg=None):
    """Prefer cfg.fps when set explicitly; otherwise fall back to the dataset default."""
    if cfg is not None:
...
def _finite_diff(x, order, fps):
    """x: [..., T, J, 3] -> [..., T-order, J, 3], already multiplied by fps**order."""
...
def compute_accel_jitter(xyz, fps, hist_xyz=None, gt_xyz=None):
    """
    xyz:      [..., T, J, 3]
    hist_xyz: [..., H, J, 3] optional, prepended to bridge the hist/pred seam
    gt_xyz:   [..., T, J, 3] optional, when provided also returns accel_error_joint
    Every returned entry has shape [..., J]
    """
...
def compute_all_metrics(pred, gt, gt_multi):
    ...
    if pred.shape[0] == 1:
        ...
        diversity = torch.pdist(pred.reshape(pred.shape[0], -1)).mean(-1).mean(-1)
...
# below. The previous versions of these lists had bugs from treating the
# dropped root's children as if they connected to each other:
...
def compute_all_metrics_detailed(pred, gt, gt_multi, num_joints, dataset_name,
    ...
    hist:
        [H, D] optional, last few history frames used to bridge the hist/pred
        seam for the acceleration/jitter computation
    """
