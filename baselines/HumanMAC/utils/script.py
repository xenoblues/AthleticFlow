...
from models.transformer_hmac import HMACMotionTransformer
...
def dataset_split(cfg):
    ...
    DATASET_CLASS_MAP = {
        ...
        "ap3d": {
            "base": DatasetAP3D,
            "multi": DatasetAP3D_multi,  # ✅ 修复bug：替换为AP3D专属多模态类
            "actions": None  # AP3D 不筛选动作
        },
        "ap": {
            "base": DatasetAthleticsPose,
            "multi": DatasetAthleticsPose_multi,  # ✅ 修复bug：替换为AP3D专属多模态类
            "actions": None  # AP3D 不筛选动作
        },
        "wp": {
            "base": DatasetWP,
...
def get_multimodal_gt_full(logger, dataset_multi_test, args, cfg):
    ...
    for data, multi_traj in data_gen_multi_test:
        ...
        traj_gt_arr.append(multi_traj[:, cfg.t_his:, 1:, :].reshape(len(multi_traj),cfg.t_pred, -1))
