...
class DatasetWP_multi(Dataset):
    def __init__(self, mode='train', t_his=25, t_pred=100, normalization=True, use_vel=False,
        ...
        self.root_path = r"/work7/y_zhou/HumanMAC"
    ...
    def _load_multimodal_data(self):
        if self.multimodal_path is None:
            multimodal_path = os.path.join(self.root_path,
                                           f'data/worldpose/multimodal/t_his{self.t_his}_1_thre0.500_t_pred{self.t_pred}_thre0.100_index_filterd.npz')
            candi_path = os.path.join(self.root_path,
                                      f'data/worldpose/multimodal/data_candi_t_his{self.t_his}_t_pred{self.t_pred}_skiprate{self.step}.npz')
        else:
        ...
        if self.data_candi.shape[1] != self.t_total:
            raise ValueError(
                f"data_candi t_total mismatch: expected {self.t_total}, got {self.data_candi.shape[1]}")
        ...
        if self.total_clips > 0 and len(self.test_clips) != self.total_clips:
            raise RuntimeError(
                f"Test clip count mismatch. Built test_clips={len(self.test_clips)}, but multimodal dict has {self.total_clips} keys. This means DatasetWP_multi and multimodal generation are not aligned.")
    ...
    def _validate_multimodal_indices(self):
        ...
        if max_idx >= len(self.data_candi):
            raise RuntimeError(
                f"multimodal index out of range: max idx={max_idx}, candidate size={len(self.data_candi)}. The index file and candidate pool are not from the same generation run.")
