...
class Config:
    ...
    def __init__(self, cfg_id, test=False, run_dir=None):
        self.id = cfg_id
        ...
        if run_dir is not None:
            # reuse an existing run directory (e.g. testing a specific trained checkpoint)
            self.cfg_dir = run_dir
        else:
            # PID suffix avoids collisions when multiple jobs start within the same second
            formatted_time = "_" + time.strftime('%Y%m%d-%H%M%S', time.localtime()) + "_%d" % os.getpid()
            self.cfg_dir = '%s/%s' % (self.base_dir, cfg_id + formatted_time)
