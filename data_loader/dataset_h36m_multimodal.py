                    ...
                    if n_modality > 0:
                        ...
                        if len(traj_multi) > 0:
                            ...
                            if traj_multi.shape[0] > n_modality:
                                ...
                                traj_multi = traj_multi[:n_modality]
                        traj_multi = np.concatenate(
                            [traj_multi, np.zeros_like(traj[[0] * (n_modality - traj_multi.shape[0])])], axis=0)
                    else:
                        traj_multi = None
...
if __name__ == '__main__':
    ...
    generator1 = dataset.iter_generator()
    seq = next(generator1)[0][:, :, 1:, :].reshape(-1, 125, 48)
    print(seq[0, 0, :5])

    dataset.normalize_data()
    generator2 = dataset.iter_generator()
    seq2 =  next(generator2)[0][:, :, 1:, :].reshape(-1, 125, 48)
    print(seq2[0, 0, :5])
    print(dataset.denormalize(seq2)[0, 0, :5])
