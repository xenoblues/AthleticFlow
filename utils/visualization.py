...
def render_animation(skeleton, poses_generator, algos, t_hist, fix_0=True, azim=0.0, output=None, mode='pred', size=2, ncol=5,
    ...
    if dataset_name == 'ap3d':
        for i in range(len(poses)):
            poses[i] = poses[i][..., [0, 2, 1]]
            poses[i][..., 2] *= -1.0
    ...
    def update_video(i):
        ...
        for n, ax in enumerate(ax_3d):
            ...
            # if dataset_name == 'ap3d':
            #     trajectories[n] = poses[n][:, 0, [0, 2, 1]]
            #     trajectories[n] = -poses[n][:, 1:]
            # else:
    ...
    def reload_poses():
        nonlocal poses
        poses = dict(filter(lambda x: x[0] in {'gt', 'context'} or x[0].startswith('gt'),
                            all_poses.items()))
    ...
    def save_figs():
        ...
        for i in range(0, t_total + 1, 10):
            if i == 0:
                ...
                update_video(i - 1)
            fig.savefig('out_svg_' + suffix + '/%d_%d.svg' % (find, i), transparent=True)
